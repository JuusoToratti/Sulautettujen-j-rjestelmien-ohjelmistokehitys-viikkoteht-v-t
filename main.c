        ...
        if (uart_poll_in(uart_dev, &c) == 0) {
            if (c == '\r' || c == '\n') {
                ...
                if (sscanf(buf, "%c,%d", &cmd, &val) == 2) {
                    // Assertti komennoille ja arvoille
                    __ASSERT(cmd == 'R' || cmd == 'Y' || cmd == 'G' || cmd == 'T', 
                            "Invalid command: %c", cmd);
                    __ASSERT(val > 0 && val < 60000, "Invalid value: %d", val);
                    
                    if (!(cmd == 'R' || cmd == 'Y' || cmd == 'G' || cmd == 'T')) {
                        __ASSERT(false, "Invalid command received via UART!");
                        DEBUG_PRINT("Ignoring invalid UART command: %c\n", cmd);
                        continue;
                    }
                    ...
                    if (cmd == 'T') {
                        ...
                        for (int r = 0; r < val; r++) {
                            __ASSERT(r < 100, "Too many repetitions: %d", r); // Suojaus ääretöntä looppia vastaan
                            ...
                            for (int i = 0; i < seq_len; i++) {
                                ...
                                if (copy) {
                                    ...
                                    __ASSERT(copy->color == 'R' || copy->color == 'Y' || copy->color == 'G',
                                            "Invalid color in copy: %c", copy->color);
                                    __ASSERT(copy->duration_ms > 0 && copy->duration_ms < 60000,
                                            "Invalid duration in copy: %d", copy->duration_ms);
                                    
                                    k_fifo_put(&seq_fifo, copy);

                                    if (seq_len < MAX_SEQ_LEN) {
                                        seq_buffer[seq_len++] = copy;
                                    } else {
                                        __ASSERT(false, "Sequence buffer overflow!");
                                    }
                                }
                            }
                        }
                        seq_len = 0; // Reset buffer after repetition
                    } else {
                        struct seq_item *item = k_malloc(sizeof(*item));
                        __ASSERT(item != NULL, "Failed to allocate memory for UART item");
                        
                        if (item) {
                            item->color = cmd;
                            item->duration_ms = val;
                            __ASSERT(item->color == 'R' || item->color == 'Y' || item->color == 'G',
                                    "Invalid color: %c", item->color);
                            __ASSERT(item->duration_ms > 0 && item->duration_ms < 60000,
                                    "Invalid duration: %d", item->duration_ms);
