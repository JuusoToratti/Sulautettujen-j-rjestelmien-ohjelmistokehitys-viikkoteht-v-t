...
bool manual_red = false;   // käsiohjaus punainen
bool manual_yellow = false; // käsiohjaus keltainen
bool manual_green = false; // käsiohjaus vihreä
bool yellow_blink_mode = false; // keltainen vilkkumistila
...
// Button 2 - Manual red control
void button_1_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    manual_red = !manual_red;
    if (manual_red) {
        gpio_pin_set_dt(&red, 1);
        printk("Button 2 pressed - Manual RED ON\n");
    } else {
        gpio_pin_set_dt(&red, 0);
        printk("Button 2 pressed - Manual RED OFF\n");
    }
}

// Button 3 - Manual yellow control
void button_2_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    ...
    if (manual_yellow) {
        ...
        printk("Button 3 pressed - Manual YELLOW ON\n");
    } else {
        gpio_pin_set_dt(&red, 0);
        gpio_pin_set_dt(&green, 0);
        printk("Button 3 pressed - Manual YELLOW OFF\n");
    }
}

// Button 4 - Manual green control
void button_3_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    manual_green = !manual_green;
    if (manual_green) {
        gpio_pin_set_dt(&green, 1);
        printk("Button 4 pressed - Manual GREEN ON\n");
    } else {
        gpio_pin_set_dt(&green, 0);
        printk("Button 4 pressed - Manual GREEN OFF\n");
    }
}
...
void button_4_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    if (!yellow_blink_mode) {
        // Aloita keltainen vilkkuminen
        prev_led_state = led_state; // tallenna nykyinen tila
        led_state = 5;
        yellow_blink_mode = true;
        printk("Button 5 pressed - Yellow blink mode ON\n");
    } else {
        // Lopeta keltainen vilkkuminen
        yellow_blink_mode = false;
        led_state = prev_led_state; // palauta edellinen tila
        // Sammuta ledit
        gpio_pin_set_dt(&red, 0);
        gpio_pin_set_dt(&green, 0);
...
void red_led_task(void *p1, void *p2, void *p3) {
    while (1) {
        ...
        // ilmoita dispatcherille että valmis
...
void yellow_led_task(void *p1, void *p2, void *p3) {
    while (1) {
        // Odota dispatcherin signaalia
        k_mutex_lock(&disp_mutex, K_FOREVER);
        k_condvar_wait(&yellow_cv, &disp_mutex, K_FOREVER);
        k_mutex_unlock(&disp_mutex);

        // Sytytä keltainen (punainen + vihreä)
        gpio_pin_set_dt(&red, 1);
        gpio_pin_set_dt(&green, 1);
        printk("YELLOW ON\n");
        k_sleep(K_SECONDS(1));
        ...
        // Ilmoita dispatcherille että on valmis
        k_condvar_signal(&release_cv);
    }
}
...
void dispatcher_task(void *p1, void *p2, void *p3) {
    while (1) {
        ...
        // Odota release-signaalia ennen seuraavaa
